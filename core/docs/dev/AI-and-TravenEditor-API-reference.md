...
### `renderMarkdown(markdownText, extraPlugins)`
...
*   **Parameters:**
    *   `markdownText` (`string`): The raw Markdown text to render.
    *   `extraPlugins` (`Array<TravenPlugin>`, optional): Host plugins appended to the default set (e.g. `ExpandEmbedPlugin`). Same contract as `options.plugins` on the editor.
